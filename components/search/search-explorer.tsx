...
export function SearchExplorer({ startups }: SearchExplorerProps) {
  ...
  const filteredStartups = useMemo(() => {
    return startups.filter((startup) => {
      ...
      if (selectedMarket !== "all") {
        if (
          startup.target_market !== selectedMarket &&
  ...
  }, [startups, query, selectedCategory, selectedMarket, selectedTag]);
      ...
      <div className="rounded-2xl border border-border bg-card p-4 shadow-apple-sm space-y-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-border/60">
          <div className="flex items-center gap-1.5">
            <span className="text-tiny font-medium text-muted-foreground mr-1">
              Market:
            </span>
            {MARKETS.map((m) => (
              <button
